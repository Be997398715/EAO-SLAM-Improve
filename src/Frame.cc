     ...
     mpReferenceKF(static_cast<KeyFrame*>(NULL))
{
    ...
    mpline_lbd_ptr_frame->detect_raw_lines(imLeft, keylines_raw);
...
Frame::Frame(   const cv::Mat &imRGB,
                ...
                const float &thDepth)
                                    :   mpORBvocabulary(voc),
                                        ...
                                        mColorImage(imRGB.clone()),      // color image.
                                        mQuadricImage(imRGB.clone()),    // quadrics image.
                                        mK(K.clone()),
                                        ...
                                        mThDepth(thDepth)
{
    ...
    // note: [EAO-SLAM] line detection.
    mpline_lbd_ptr_frame->detect_raw_lines(imRGB, keylines_raw);
    mpline_lbd_ptr_frame->filter_lines(keylines_raw, keylines_out);
...
void Frame::ComputeImageBounds()
{
    if(mDistCoef.at<float>(0)!=0.0)
    {
        cv::Mat mat(4,2,CV_32F);
        mat.at<float>(0,0)=0.0; mat.at<float>(0,1)=0.0;
        mat.at<float>(1,0)=mImGray.cols; mat.at<float>(1,1)=0.0;
        mat.at<float>(2,0)=0.0; mat.at<float>(2,1)=mImGray.rows;
        mat.at<float>(3,0)=mImGray.cols; mat.at<float>(3,1)=mImGray.rows;
        ...
        // make sure it is inside image
        mnMinX = max(mnMinX,0.0f);
        mnMaxX = min(mnMaxX,(float)mImGray.cols);
        mnMinY = max(mnMinY,0.0f);
        mnMaxY = min(mnMaxY,(float)mImGray.rows);
