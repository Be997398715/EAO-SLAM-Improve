...
/*export*/ template <class T>
Matrix_<T>::Matrix_(const std::initializer_list<std::initializer_list<T>> init) {
  m_matrix = nullptr;
  m_rows = init.size();
  if ( m_rows == 0 ) {
    m_columns = 0;
...
/*export*/ template <class T>
Matrix_<T>::Matrix_(const Matrix_<T> &other) {
...
/*export*/ template <class T>
Matrix_<T>::Matrix_(const size_t rows, const size_t columns) {
  m_matrix = nullptr;
  resize(rows, columns);
}
...
/*export*/ template <class T>
Matrix_<T> &
Matrix_<T>::operator= (const Matrix_<T> &other) {
  if ( other.m_matrix != nullptr ) {
    // copy arrays
    resize(other.m_rows, other.m_columns);
    for ( size_t i = 0 ; i < m_rows ; i++ ) {
...
/*export*/ template <class T>
void
Matrix_<T>::resize(const size_t rows, const size_t columns, const T default_value) {
...
/*export*/ template <class T>
inline T&
Matrix_<T>::operator ()(const size_t x, const size_t y) {
  assert ( x < m_rows );
  assert ( y < m_columns );
  assert ( m_matrix != nullptr );
  return m_matrix[x][y];
...
/*export*/ template <class T>
inline const T&
Matrix_<T>::operator ()(const size_t x, const size_t y) const {
  assert ( x < m_rows );
  assert ( y < m_columns );
  assert ( m_matrix != nullptr );
  return m_matrix[x][y];
