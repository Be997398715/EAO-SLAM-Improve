...
public:
  Matrix_();
  Matrix_(const size_t rows, const size_t columns);
  Matrix_(const std::initializer_list<std::initializer_list<T>> init);
  Matrix_(const Matrix_<T> &other);
  ...
  friend std::ostream& operator<<(std::ostream& os, const Matrix_ &matrix)
  {
      os << "Matrix:" << std::endl;
