...
Tracking::Tracking( System *pSys, ORBVocabulary *pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Map *pMap,
                    ...
                    mpPointCloudMapping( pPointCloud ), mState(NO_IMAGES_YET), mSensor(sensor), mbOnlyTracking(false), mbVO(false), mpORBVocabulary(pVoc),
                                                                                                                                                                                              ...
                                                                                                                                                                                              mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    */
    ...
    //yolov3 detector and deepsort tracker init
    YOLO detector;
    cout << "Detector YOLO Create!" <<endl;
}
...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB, const cv::Mat &imD, const double &timestamp, const bool bSemanticOnline)
{
    ...
    mCurrentFrame = Frame(imRGB,         // color image.
                          imDepth,
                          mImGray,
                          timestamp,
                          mpORBextractorLeft,
                          ...
                          mbf,
                          mThDepth);
    //cout << "tracking with GrabImageRGBD Frame" << endl;
                                     ...
                                     const bool bSemanticOnline)
{
    ...
    else
    {
        ...
        for (auto &mat_row : _mat)
        {
            ...
            box.m_class = mat_row[0];
            box.m_score = mat_row[5];
            box.m_track_id = -1; //offline has no track id
            box.x = mat_row[1];
            box.y = mat_row[2];
            box.width = mat_row[3];
            box.height = mat_row[4];
...
void Tracking::StereoInitialization()
{
    if (mCurrentFrame.N > 500)
    {
        ...
        mState = OK;
        cout << "RGBD Init Successfully!" << endl;
    }
}
...
{
    cout <<"TrackReferenceKeyFrame start" <<endl;
...
bool Tracking::TrackWithMotionModel()
{
    ...
    //cout << "construct bounding box by feature points END "<<endl;
    // **********************************************************************************************
...
void Tracking::CreateNewKeyFrame(bool CreateByObjs)
{
    ...
    if(mSensor==System::RGBD || mSensor==System::STEREO)
        pKF = new KeyFrame(mCurrentFrame, mpMap, mpKeyFrameDB, this->mImRGB, this->mImDep);
    else
    ...
    if(mSensor!=System::MONOCULAR)
        mpPointCloudMapping->insertKeyFrame( pKF, this->mImGray, this->mImDep );
            ...
            if (!pMP->isBad())
            {
                for (size_t k = 0; k < objs_2d.size(); ++k)
                {
                    if (objs_2d[k]->mBoxRect.contains(mCurrentFrame.mvKeysUn[i].pt))// in rect.
                    {
                        ...
                        pMP->track_id = objs_2d[k]->_track_id;
                        // object points.
...
void Tracking::InitObjMap(vector<Object_2D *> objs_2d)
{
    ...
    for (auto &obj : objs_2d)
    {
        ...
        for (size_t i = 0; i < obj->Obj_c_MapPonits.size(); i++)
        {
            ...
            cv::Mat loc = pMP->GetWorldPos();
            if(loc.empty()){
                cout << "empty pos:"<< loc<<  endl;
                continue;
            }
