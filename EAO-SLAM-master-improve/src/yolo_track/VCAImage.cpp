...
void ShowTagColor(Mat &input_image, Point *p_pt, const char *p_msg_buffer, Scalar color, bool show_tag_bg)
{
	...
	if(show_tag_bg)
	{
		rectangle(input_image,msg_rect,color,-1, LINE_AA);
		putText(input_image, p_msg_buffer, Point(msg_rect.x,msg_rect.y+9),FONT_HERSHEY_SIMPLEX,(double)0.3,Scalar(0,0,0),1,LINE_AA);
	}
	else
	{
		putText(input_image, p_msg_buffer, Point(msg_rect.x,msg_rect.y+9),FONT_HERSHEY_SIMPLEX,(double)0.3,color,1,LINE_AA);
	}
}
...
void ShowTagRMCT(Mat &input_image, Rect o_rect, const char *p_msg_buffer, Scalar o_scalar, int thickness, bool show_position, bool show_tag_bg, int tag_position)
{
	...
	rectangle(input_image, o_rect, o_scalar, thickness, LINE_AA);
...
void ShowLine(Mat &input_image, Point pt1, int len, int direction, Scalar o_scalar, int thickness)
{
	...
	if(direction&ShowLine_LEFT)
	{
		pt2.x=pt1.x-len;
		pt2.y=pt1.y;
		line(input_image, pt1, pt2, o_scalar, thickness, LINE_AA);
	}
...
{
	line(input_image, RP1(trail1), RP1(trail2), o_scalar, thickness, LINE_AA);
	line(input_image, RP2(trail1), RP2(trail2), o_scalar, thickness, LINE_AA);
	line(input_image, RP3(trail1), RP3(trail2), o_scalar, thickness, LINE_AA);
	line(input_image, RP4(trail1), RP4(trail2), o_scalar, thickness, LINE_AA);
}
