...
LocalMapping::LocalMapping(Map *pMap, const float bMonocular, const string &flag):
    mbMonocular(bMonocular), mbResetRequested(false), mbFinishRequested(false), mbFinished(true), mpMap(pMap),
    mbAbortBA(false), mbStopped(false), mbStopRequested(false), mbNotStop(false), mbAcceptKeyFrames(true), mflag(flag)
{
}
...
void LocalMapping::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            //NOTE [EAO-SLAM] update object map.
            UpdateObject();
...
// BRIEF [EAO-SLAM] merge potentially associated objects.
void LocalMapping::MergePotentialAssObjs()
{
    ...
    for(int i = 0; i < (int)vObjs.size(); ++i)
    {
        ...
        if (Obj->mObjectFrame.size() >= 10)     // todo: when to do ?
        {
            ...
            {
                Obj->WhetherMergeTwoMapObjs(mpMap);
            }
        }
    }
