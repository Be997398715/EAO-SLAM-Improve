...
void Object_2D::CopyBoxes(const BoxSE &box)
{
    ...
    #ifdef DEEP_SORT
        mScore = box.m_score>0.5?box.m_score:box.m_score+0.2;
    #else
        mScore = box.m_score;
    #endif
    // box.
...
void Object_2D::ObjectDataAssociation(Map *mpMap, Frame &mCurrentFrame, cv::Mat &image, string &flag)
{
    ...
    // ****************************************************************************
    //         STEP 0. Tracker data association.(sort-no use, deepsort), combine iou to get proper score              *
    // ****************************************************************************
    #ifdef DEEP_SORT
        ...
        if ((IouMax > 0) && (IouMaxObjID >= 0) && (track_score>=TrackThreshhold))
        {
            ...
            if (bFlag)
            {
                ...
                "\nObjID=" << IouMaxObjID << "\ntrack score="<<track_score <<endl<< endl;
            }
        }
        ...
        if((flag != "NA") && (flag != "NP"))
        {
            ...
            if ((IouMax > 0) && (IouMaxObjID >= 0))
            {
                ...
                if (bFlag)
                {
                    bAssoByIou = true;              // associated by IoU.
                    nAssoByIouId = IouMaxObjID;     // associated map object id.
                    cout << "association by IOU!\n" <<  "IouMax="<<IouMax<<
                     "\nObjID=" << IouMaxObjID << endl;
    ...
    #endif
cout <<"STEP 2. Nonparametric data association" << endl;
    // *************************************************
...
cout <<"STEP 3. Projected box data association" << endl;
    if (old == false)
    {
        // ****************************************************
...
cout <<"t-test data association" << endl;
        // ************************************************
...
cout <<"STEP 4. create a new object" << endl;
        // *************************************************
        //             STEP 4. create a new object         *
        // *************************************************
        #ifdef DEEP_SORT
            if (bAssoByIou || bAssoByNp || bAssoByProject || bAssoByT || bAssoByTracker)
                return;
        #else
                                     ...
                                     int Flag) // 1 Iou, 2 NP, 3 t-test，4 project. for debug.
{
    ...
    for (size_t j = 0; j < ObjectFrame->Obj_c_MapPonits.size(); ++j)
    {
        ...
        else
        {
            ...
            for (size_t m = 0; m < mvpMapObjectMappoints.size(); ++m)
            {
                ...
                cout << obj_curr_pos << obj_map_pos<< endl;
                if (obj_map_pos.empty()){
                    cout << "obj_map_pos empty!"<< endl;
                    continue;
                }    //fix a bug leading to error when obj_map_pos is empty
        ...
        (ObjectFrame->mBox.y + ObjectFrame->mBox.height < image.rows - 25))
    {
             ...
             pMP != mvpMapObjectMappoints.end();)
        {
            ...
            cv::Mat PointPosWorld = (*pMP)->GetWorldPos();
            cout << PointPosWorld <<endl;
            if(PointPosWorld.empty())
                continue;
            cv::Mat PointPosCamera = Rcw * PointPosWorld + tcw;
