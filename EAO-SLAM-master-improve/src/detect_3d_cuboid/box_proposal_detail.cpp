										...
										std::vector<ObjectSet>& all_object_cuboids)
{
    ...
    if(whether_plot_detail_images)
    {
		...
		cv::namedWindow("Raw detected Edges");
		//moveWindow("Raw detected Edges", 20, 300);
    ...
    for (int object_id = 0; object_id < num_2d_objs; object_id++)
    {
		...
		for (int sample_down_expan_id = 0; sample_down_expan_id < (int)down_expand_sample_all.size(); sample_down_expan_id++)
		{
			...
			for (int ii = 0; ii < (int)top_x_samples.size(); ii++)
			{
			...
			// int obj_yaw_id = 8;
			for (int cam_roll_id = 0; cam_roll_id < (int)cam_roll_samples.size(); cam_roll_id++)
			for (int cam_pitch_id = 0; cam_pitch_id < (int)cam_pitch_samples.size(); cam_pitch_id++)
			for (int obj_yaw_id = 0; obj_yaw_id < (int)obj_yaw_samples.size(); obj_yaw_id++)
			{
				if (whether_sample_cam_roll_pitch)
				{
				...
				MatrixXd all_vp_bound_edge_angles = VP_support_edge_infos(	all_vps, 				
																			edge_mid_pts,			
																			lines_inobj_angles,		
				...
				for (int sample_top_pt_id = 0; sample_top_pt_id < sample_top_pts.cols(); sample_top_pt_id++)
				{
					...
					for (int config_id = 1; config_id < 3; config_id++)  // configuration one or two of matlab version
					{
						...
						double total_angle_diff = box_edge_alignment_angle_error(	all_vp_bound_edge_angles,	
																					vps_box_edge_pt_ids,		
																					box_corners_2d_float);		

						all_configs_error_one_objH.row(valid_config_number_one_objH).head<4>() = Vector4d(	config_id, 			
																											vp_1_position, 		
																											obj_yaw_esti, 		
																											sample_top_pt_id);	
						all_configs_error_one_objH.row(valid_config_number_one_objH).segment<3>(4) = Vector3d(	sum_dist/obj_diaglength_expan, 	
																												total_angle_diff, 				
																												down_expand_sample);			

						if (whether_sample_cam_roll_pitch)
							all_configs_error_one_objH.row(valid_config_number_one_objH).segment<2>(7) = Vector2d(	cam_roll_samples[cam_roll_id],		
			...
			fuse_normalize_scores_v2(	all_configs_error_one_objH.col(4).head(valid_config_number_one_objH), 	
										...
										good_proposal_ids, 				
										weight_vp_angle,				
										whether_normalize_two_errors);	

			for (int box_id = 0; box_id < (int)good_proposal_ids.size(); box_id++)
			{
			...
			for (int box_id = 0; box_id < (int)raw_obj_proposals.size(); box_id++)
			{
			...
			sort_indexes(	all_combined_score,
							sort_idx_small,				
		...
		//if (whether_plot_final_images || whether_save_final_images)
		{
			...
			for (int object_id = 0; object_id < (int)all_object_cuboids.size(); object_id++)
			...
			if (whether_save_final_images)
				...
				cv::imwrite("/home/wu/result.jpg",cuboids_2d_img);
